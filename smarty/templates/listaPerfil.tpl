							...
							<div class="x_content">
								...
								<a href="perfil.php">
									...
									<table border="" class="table table-striped responsive-utilities jambo_table" id="table-tela-lista">
										...
										<tbody>
										{foreach from=$perfilArray item=saida name=transaction_loop}
											<tr {if $smarty.foreach.transaction_loop.index%2} class="odd pointer" {else} class="even pointer" {/if}>
												...
												<td>{$saida->getPontosFortes()}</td>
												<td>{$saida->getPontosMelhoria()}</td>
												<td>{$saida->getValores()}</td>
												<td>{$saida->getDescricao()}</td>
												<td style="text-align: center">
												<a href="atualizar_perfil.php?Id={$saida->getPerfilComportamental()}">	
													<button type="submit" class="btn btn-primary">
														<span class="glyphicon glyphicon-pencil" aria-hidden="true"></span>	
													</button>
												</a>
												<button type="button" class="btn btn-danger"  onclick= "excluir({$saida->getPerfilComportamental()})">
