							...
							<div class="x_content">
								...
								<a href="configuracao.php">
									...
									<table border="" class="table table-striped responsive-utilities jambo_table"  id="table-tela-lista">
										...
										<tbody>
											{foreach from=$configArray item=saida name=transaction_loop}
											<tr {if $smarty.foreach.transaction_loop.index%2 } class="odd pointer" {else} class="even pointer" {/if} >
												<td style="height: 30px" > {$saida->getKeySystem()}</td>
												<td> {$saida->getValueSystem()} </td>
												<td> {$saida->getContext()} </td>
												<td style="text-align: center">
													<a href="atualizar_configuracao.php?Id={$saida->getConfiguracao()}">
														<button type="submit" class="btn btn-primary">
															<span class="glyphicon glyphicon-pencil" aria-hidden="true"></span>	
														</button>
													</a>
													<button type="button" class="btn btn-danger"  onclick= "excluir({$saida->getConfiguracao()})">
