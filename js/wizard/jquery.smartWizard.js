        ...
        $($this.buttons.finish).click(function() {
            if(!$(this).hasClass('buttonDisabled')){
                if($.isFunction($this.options.onFinish)) {
                	...
                	var error = 0;
                	var naoMarcou = 0;
                	var index = $this.curStepIdx + 1;
                	var sizeRadios;

                	$('#step-'+index+' input[type=radio]').each(function() {
                		sizeRadios =  $('#step-'+index+' input[name=' + this.name + ']').size();
            			$('#step-'+index+' input[name=' + this.name + ']').each(function() {
            				if (!(this.checked)){
            	        		naoMarcou++;
            	        	}
            				
            				if(naoMarcou == sizeRadios){
            					error++;
            				}
                	...
                	if ( $('input[type=radio]:checked').val() && error == 0 )
                	{
